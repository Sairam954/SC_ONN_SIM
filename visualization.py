...
def plotAndSaveBarplot(df, x_column, y_column, y_axis_label ,hue_column, ncol, file_name, fig_save_dir):
    ...
    # ax.set_ylim(0,100)
    if y_column=="fps_per_w_per_area" or y_column=="fps_per_w":
...
# df3 = calGmeanDF(df3,cal_gmean_col)
# * Dataframe to be ploted 
# df =  pd.concat([df1, df2, df3], ignore_index=True)
# df.to_csv('Result/'+acc_precision+'/Combined.csv')

df = pd.read_csv('Result/'+acc_precision+'/Combined.csv')
...
for metric in parameters_label:
        ...
        plotAndSaveBarplot(df,'Model_Name',metric,parameters_label[metric],'name', 5,fileName ,'Plots/'+acc_precision+'/')
...
for idx in df_descriptive.index:
    ...
    print("The accelerator "+ str(max_tpc_name)+ " achieves "+str(increment)+"x times better "+parameter_column+" than "+str(df_descriptive['name'][idx]))

df_descriptive= df_descriptive.drop(df_descriptive[df_descriptive['name'] == max_tpc_name].index)
