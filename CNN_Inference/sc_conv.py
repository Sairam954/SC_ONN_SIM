...
class Conv2d(_ConvNd):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        # self.proposed_lut = pd.read_csv('C:\\Users\\SSR226\\Desktop\\CNN_Inference_Sim\\.venv\\Lib\\site-packages\\torch\\ao\\nn\\quantized\\modules\\8_bit_lut.csv')
        self.lut = np.load('C:\\Users\\SSR226\\Desktop\\CNN_Inference_Sim\\.venv\\Lib\\site-packages\\torch\\ao\\nn\\quantized\\modules\\sc_lut.npy')
    ...
    def stochasticDotProduct(self,dVec1,dVec2, unary_converter='proposed'):
        # convert binary vectors to unary vector
        
        bDotProdRes = self.lut[dVec1+255, dVec2+255]
        bDotProdRes = bDotProdRes*256
        # if unary_converter == 'proposed':
        #     lut = self.proposed_lut
        #     bDotProdRes = 0
        #     sign = 1
        #     for element_no in range(len(dVec1)):
        #         dValue1 = dVec1[element_no].item()
        #         dValue2 = dVec2[element_no].item()
        #         if dValue1 < 0 and dValue2 < 0:
        #             dValue1 = abs(dValue1)
        #             dValue2 = abs(dValue2)
        #             sign = 1
        #         elif dValue1 < 0 or dValue2 < 0:
        #             dValue1 = abs(dValue1)
        #             dValue2 = abs(dValue2)
        #             sign = -1 
                        
        #         uResRow = lut.query("`Decimal_OP1` == @dValue1 and `Decimal_OP2` == @dValue2")
        #         bMulRes = uResRow['SC_DC_OPT'].values[0]
        #         # uMulRes = torch.tensor(uMulRes,dtype=torch.int8)
        #         bDotProdRes += bMulRes*sign
        
        return sum(bDotProdRes)

    def matmul(self,mat1, mat2):
        # size = (batch_size, no_of_channels, H, W)
        # mat1 = torch.randint(0,7,size=(1,1,3,3))
        # mat2 = torch.randint(0,7,size=(1,1,3,3))
       
        mat1 = mat1.to(torch.int8)
        mat2 = mat2.to(torch.int8) 
        mat1_dim = mat1.shape
        mat2_dim = mat2.shape
        # print("Mat 1 Dim", mat1.shape)
        # print("Mat 2 Dim", mat2.shape)
        # batch_size = mat1_dim[0]
        no_of_channels = mat1_dim[0]
        no_of_rows = mat1_dim[1]
        no_of_columns = mat2_dim[1]

        # matrix multiplication output dimension = (batch size, no_of_channels, row count of first mat, column count of second mat)
        matmul_result = torch.zeros(size=(no_of_channels,no_of_rows,no_of_columns))

        # print(mat2)
        # print('Total Row ', no_of_rows)
        # iterate over the batch
        # for batch_no  in range(batch_size):
        for channel_no in range(no_of_channels):
            for row in range(no_of_rows):
                # print('Row ', row)
                for col in range(no_of_columns):
                    dotprod = self.stochasticDotProduct(mat1[channel_no,row,:], mat2[:,col])
                    matmul_result[channel_no,row,col] = dotprod
    ...
    def conv2d_output_shape(self, h_w, kernel_size=1, stride=1, pad=0, dilation=1):
        h_w, kernel_size, stride, pad, dilation = (
            self.num2tuple(h_w),
            self.num2tuple(kernel_size),
            self.num2tuple(stride),
            self.num2tuple(pad),
            self.num2tuple(dilation),
        )
    ...
    def forward(self, input):
        ...
        if self.padding_mode != 'zeros':
            ...
            input = F.pad(input, _reversed_padding_repeated_twice,
                          mode=self.padding_mode)
        # print("Convolution 2D Forward Method")
        # print("Before Unfold")
        # print('Kernel Height', self.kernel_h)
        # print('Kernel Weight', self.kernel_w)
        conv_output_shape = self.conv2d_output_shape(
            (input.size()[2], input.size()[3]),
            kernel_size=self.kernel_h,
            dilation=self.dilation,
            pad=self.padding,
            stride=self.stride,
        )
        qinput = input.int_repr()
        qinput = qinput.float()
        ...
        # print("After Unfold")
        #  print(unfolded)
        # unfolded = torch.dequantize(unfolded)
        # print(unfolded)
        # print(self.weight())
        weight = self.weight().int_repr()
        # print("Dequantize Input", qinput)
        # print("Dequantize Weight", weight)
        conv_output = (
            
            self.matmul(unfolded.transpose(1, 2), weight.view(weight.size(0), -1).t())
            .transpose(1, 2)
        )
        # print('======================================')
        ...
        # print('In built Convolution Shape', output.int_repr())
        conv_output_fold = torch.nn.functional.fold(
            conv_output, conv_output_shape, (1, 1)
        )
        output = torch.quantize_per_tensor(conv_output_fold, self.scale, self.zero_point, torch.quint8)
        
        print('Stochastic Convolution Shape')
